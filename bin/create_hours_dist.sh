...
header_file="$current_dir/html_components/hours_dist_header.html"
footer_file="$current_dir/html_components/hours_dist_footer.html"
...
find . -mindepth 1 -maxdepth 1 -type d | while read -r dir; do
  ...
  if [ -f "$failed_login_file" ]; then
    awk '{print $3}' "$failed_login_file" >> "$temp_file"
  fi
done

if [ -s "$temp_file" ]; then
  sort "$temp_file" | uniq -c | while read -r occurrences hour; do
    printf "data.addRow([\x27%s\x27, %d]);\n" "$hour" "$occurrences" >> "$output_file"
  done
else
  echo "No hours data found."
  exit 1
fi
